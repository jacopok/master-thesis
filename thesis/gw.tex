...
\section{Parameters for a \ac{CBC} waveform} \label{sec:cbc-parameters}
...
Qualitatively, a waveform from a \ac{CBC} involves a long \emph{inspiral} phase which terminates when the two compact bodies \emph{merge}, and finally a \emph{post-merger} phase in which the single body remaining exhibits damped vibrations.
The frequencies associated to these phases are rising: as an order-of-magnitude estimate, the merger frequency is close to \(Mf = 0.02 \divisionsymbol 0.03\) \cite{bernuzziModelingDynamicsTidallyinteracting2015}, which corresponds to roughly \SI{1.5}{kHz} to \SI{2}{kHz} for a \ac{BNS} system with \(M = 2.8 M_{\odot}\). 

The inspiral is associated to lower frequencies than these, while the post-merger is associated with higher ones.
The high-frequency regime, including the late inspiral, the merger and the post-merger, is where the Newtonian approximation breaks down.
