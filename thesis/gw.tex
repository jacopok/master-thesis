...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/polarizations}
\caption{Polarizations of gravitational waves. Time evolution is represented though transparency: darker ellipses correspond to later points in time. The magnitude of the GW effect is \(30\%\) of the radius of the ellipse. The four cases are: \(h_+\) only is nonzero, \(h_\times \) only is nonzero; \(h_\times = i h_+\) for the right polarization \(h_R\), \(h_\times = - i h_+\) for the left polarization \(h_L\); in the latter two cases the magnitude of both is also divided by \(\sqrt{2}\) for normalization. We stress that in the TT gauge the ellipses do not represent changes in coordinate position but only in distance among points. The code generating this figure is available \href{https://github.com/jacopok/master-thesis/blob/main/thesis/figures/polarizations.py}{here}.}
...
\end{align}
%
where the tensor \(\Lambda \) is constructed to be a fully traceless, fully transverse projector for 2-tensors: 
%
\begin{align}
