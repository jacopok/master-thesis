...
\subsubsection{Arrival time and initial phase}
...
%
\begin{align}
\int_{\mathbb{R}} f(t-t_0 ) e^{i \varphi_0 } e^{i \omega t} \dd{t}  = e^{i \omega t_0 + i \varphi_0} \int_{\mathbb{R}} f(t-t_0 ) e^{i \omega (t-t_0 )} \dd{(t-t_0 )}
\,,
\end{align}
%
...
\subsubsection{Sky position and polarization}
...
\subsubsection{Masses and mass ratio}

The waveform to the lowest order we described only depends on the chirp mass \(\mathcal{M}_c\), but as we will see later at higher order there appears a dependence on the
