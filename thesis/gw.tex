...
% \paragraph{Spherical harmonics decomposition}
...
% In general one can write the waveform as 
% %
% \begin{align}
% h_+ + i h_\times = \frac{1}{r} \sum _{l=2}^{\infty } \sum _{m=-\ell}^{\ell} h_{\ell m} {}_{-2}Y_{\ell m}
% \,,
% \end{align}
% %
...
Specifically, we use the fact that in an inner product space we can write the product as \((a|b) = \sqrt{(a|a) (b|b)} \cos \theta \), where \(\theta \) can be interpreted as an angle between the \(a\) and \(b\). 
Therefore, we can define the \emph{mismatch} ``distance''\footnote{This is not a distance in the mathematical sense: it does not satisfy the identity of indiscernible principle since \(M(a, Ka) = 0\) for constant \(a\), and it does not satisfy the triangle inequality \(M(a, b) \leq M(a, c) + M(b, c)\) --- think, for example, of \(a\) and \(c\) at right angles to one another (so \(M(a, c) = 1\)) and \(b\) in the middle, at 45° from either, so \(M(a, b) = M(b, c) = 1 - \sqrt{2} / 2 \approx 0.3\). Then, \(M(a,c) = 1 > M(a, b) + M(b, c) \approx 0.6\), in violation of the inequality. 

This is not a problem for us: this mismatch is still a useful metric even if it is not a distance. }
%
\begin{align}
M(a, b) = 1 - \frac{(a|b)}{\sqrt{(a|a) (b|b)}} = 1 - \cos \theta  \in [0, 2]
\,.
\end{align}
...
%
\begin{align}
\mathcal{M}(a, b) = 1 - \max_{\phi_0 , t_0 } \frac{(a|b(\phi_0, t_0 ))}{\sqrt{(a|a)(b|b)}}
\,,
\end{align}
%
where either of the two waveforms is shifted in time and in phase.
This is sometimes also denoted as \(F\), the \emph{fidelity}. 
\todo[inline]{Is this true? Should check on that. Maybe typically \(M = \cos( \theta )\) and \(F = 1- \cos \theta \)\dots}
...
&= \max_{\phi_0, t_0 } 4 \abs{\int_{0}^{\infty } \dd{f} \frac{A_a (f) A_b(f) e^{i \phi_a - i \phi _b + 2 \pi i f t_0 }}{S_n (f)}}
\,.
\end{align}
...
We then have a closed-form expression for our posterior probability density, so if computational power were no issue we would be done.
The function \(\mathbb{P}(\theta | s)\) includes all the possible information we could want about the parameters. 
We can extract \emph{estimators} for single parameters and their variances in order to better understand what we have measured: these are functions which take the full posterior and return an estimate for a single quantity, such as the value for a specific parameter. We ask of these estimators that they be 
\begin{enumerate}
...
Depending on the way the posterior distribution is calculated and parametrized, this may be impractical.
Other choices include maximizing the likelihood or the posterior. 
...
\State with probability \(\alpha \), or if \(\alpha >1\), accept
\If{accept}
...
This effect comes about at the 2.5PN order: equating the emitted power estimated in equation \eqref{eq:energy-emission-estimate} to the derivative of the energy (which by the virial theorem has the same magnitude as the kinetic energy) \(\dot{E} = - M v \dot{v}\) we find  \(\dot{v} \propto (v/c)^{5}\). 
...
This allows us to turn the equations to solve into Poisson equations in the form 
%
\begin{align}
...
The error in the phase is therefore of order \(x^{-1/2} > 1\) if we go to 2PN order, and we need a high-order computation in order to recover an accurate expression for the phase. 
...
This is possible but time-consuming: each simulation requires days of supercomputer time, while thousands of waveforms are needed for the analysis of a signal. 
...
One must therefore find a way to move to the frequency domain. An option is to use a \ac{SPA} \cite{gambaFastFaithfulFrequencydomain2020}; this has been found to be quite successful in providing fast waveform evaluations even for low initial frequencies, as can be seen in figure \ref{fig:TEOBResumSPA_evaluation}. 
...
In order to perform an \ac{FFT}, we need equally-spaced points in time, so the full duration needs to be densely sampled; also, the merger moment needs to be resolved by the sampling density. 
This constrains the sampling rate to be quite high, 
