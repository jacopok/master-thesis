...
\dv{E}{t} = -\frac{G}{5} \expval{ \dv[3]{Q_{ij}}{t} \dv[3]{Q_{ij}}{t}}
\,.
\end{align}

Dimensionally, \(Q \sim \int \dd[3]{y} \rho r^2\) has units of \SI{}{kg m^2}; so \(\dv*[3]{Q}{t}\) has units of \(\SI{}{kg m^2 / s^3}\): Watts.
...
This means that in order for \ac{GW} emission to be efficient we must have a large value for \(\dv*[3]{Q}{t}\): let us estimate it in terms of the typical size of the system, \(R\), of its typical velocity \(v = \Omega R\) and of its mass \(M\). 
Each time derivative will roughly correspond to multiplication by a factor \(\Omega \), so \(\dv*[3]{Q}{t} \sim \Omega^3 M R^2 = v^3 M / R\).
...
%
\begin{align}
\dv{L^{i}}{t} = - \frac{2G}{5c^{5}} \epsilon^{ikl} \expval{ \dv[2]{M_{ka}}{t} \dv[3]{M_{la}}{t}} 
\,.
\end{align}
...
%
\begin{align} \label{eq:optimal-mismatch}
...
The high-frequency regime, including the late stages of the inspiral, the merger and the post-merger, is where the Newtonian approximation breaks down.
