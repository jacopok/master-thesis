...
\subsubsection{Natural units and simplifications} \label{sec:natural-units}
% fix title! 
...
We shall use it to evaluate the accuracy of our waveform templates against certain reference templates; what do we ask of our templates \cite{lindblomModelWaveformAccuracy2008}? 

\begin{enumerate}
    \item In a matched-filtering search for signals, they need to be accurate enough so that we do not miss a large fraction of the signals;
    \item in the data analysis of a signal, they need to be accurate enough that they do not induce a significant bias in the estimation of parameters.
\end{enumerate}

It turns out that, indeed, the fidelity distance is the correct metric to evaluate both of these criteria. 
In the matched-filtering context, the fraction of true signals we miss is also related both to the density of our template bank, but 
