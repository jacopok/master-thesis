...
% This may seem like a reason to switch to a lower-order interpolation mechanism, but it is balanced by the fact that the lower number of points allows for a much larger training dataset, and faster evaluations of the \ac{PCA} + \ac{NN} ensemble.
...
% Here we will not use it for this purpose though, it will be more of a pre-processing step. 
...
% \ac{PCA} optimizes Euclidean distance, so the scaling of the amplitude and phase residuals becomes relevant. 
