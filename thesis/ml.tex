...
the one which is relevant for us here is known as \emph{supervised learning}. 
In general, the idea is to start from a set of training data consisting of pairs \((x, y)\), 
...
If we indicate \(C_V(\alpha ; \beta )\) as the error on the validation data, the full procedure can then be schematically be written as 
%
\begin{align}
...
The best way to calculate these two optimums differ, both among each other and on a case-by-case basis: the validation procedure must account for the fact that the training procedure for each value of \(\beta \) analyzed can take some time. 
...
\subsection{Greedy approximants}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/PCA_vary_K}
\caption{The reconstruction error is computed after projecting onto a \(K\)-dimensional subspace a number \(N_w=256\) waveforms. The parameters for the \ac{PCA} model are fitted with an independent, \(N _{\text{PCA}} = 1024\) waveform dataset. A \ac{KDE} is performed for each set of corresponding log-fidelities, in order for the probability curve to look smoother.
Here the \ac{EOB} waveforms are still calculated with a \ac{SPA}. 
}
