...
=======
    = \int_{0}^{\infty } \dd{f} S_n(f)^{\text{single-sided}}
    = \int_{-\infty }^{\infty } \dd{f} S_n(f)^{\text{double-sided}}
>>>>>>> 1c60f50814633886e0a2e899d589000c73b1d2e6
    \,.
    \end{align}
...
\end{align}
%
where we defined the \textbf{Wiener product} between two real-valued signals \(a\) and \(b\) as the Fourier-space expression
%
\begin{align}
...
%
\begin{align}
a_w(t) \qquad \text{such that} \qquad \widetilde{a}_w (f) = \frac{\widetilde{a}(f)}{\sqrt{S_n(f)}}
\,.
\end{align}
...
=======
    } \textbf{power spectral density} \(\expval{\widetilde{n}^{*}(f ) n(f )} = \delta (f - f' ) S_n(f ) / 2\). 
    \item Gaussian: each Fourier component is normally distributed around zero, with a variance described by the \ac{PSD}.
\end{enumerate}

The \ac{PSD}, as defined, has the dimension of an inverse frequency; since it describes a variance it is often useful to discuss its square root, the \emph{spectral strain sensitivity} \(\sqrt{S_n}\), with dimensions \(1/ \sqrt{ \SI{}{Hz}}\). 
>>>>>>> 1c60f50814633886e0a2e899d589000c73b1d2e6
